...
def tth(cx,cy,cz,rx,ry,rz):
    theta=0.5*np.arctan2(np.sqrt(pow(np.dot(np.dot(matrotxyz(rx,ry,rz),np.array([cx,cy,cz])),vy),2)+pow(np.dot(np.dot(matrotxyz(rx,ry,rz),np.array([cx,cy,cz])),vz),2)),np.dot(np.dot(matrotxyz(rx,ry,rz),np.array([cx,cy,cz])),vx))
    return theta
    
def azim(cx,cy,cz,rx,ry,rz):
    azimuth=np.arctan2(np.dot(np.dot(matrotxyz(rx,ry,rz),np.array([cx,cy,cz])),vz),np.dot(np.dot(matrotxyz(rx,ry,rz),np.array([cx,cy,cz])),vy))
    return azimuth
