...
#fielSave = '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_align_NEW.h5'
#sample = 'align'
#dataset = '0001'
...
#{
#  "paramFilename" : "/home/esrf/slim/easistrain/easistrain/EDD/calibrationEdd.json",
#  'fileRead': '/home/esrf/slim/data/ihme10/id15/align/ihme10_align.h5',
#  'fileSave': '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_align_NEW.h5',
#  'sample' : 'align',
#  'dataset' : '0001',
#  'detectorScanNumber' : [66,65],
#  'namesDetectors' : ['mca2_det0', 'mca2_det1'],
#  'numberOfBoxes': 4,
#  'nbPeaksInBoxes ' : [1, 2, 1, 1],
#  'rangeFit' : [620,780,1020,1120,3500,3800,3850,4090,620,780,1020,1120,3500,3800,3850,4090]
#  'sourceCalibrantFile' : '/home/esrf/slim/easistrain/easistrain/EDD/BaSource' 
#}


def splitPseudoVoigt(
	xData,
	*params
	):
	return silx.math.fit.sum_splitpvoigt(xData, *params)

def gaussEstimation(xData,
	*params
	):
...
def calcBackground(
	...
	if (int(guessedPeaksIndex[0] - 3 * fwhmLeft) < 0 and 
		int(guessedPeaksIndex[-1] + 3 * fwhmRight) <= len(xData)): ## case of not enough of points at left
	...
	if (int(guessedPeaksIndex[-1] + 3 * fwhmRight) > len(xData) and 
		int(guessedPeaksIndex[0] - 3 * fwhmLeft) >= 0): ## case of not enough of points at right
	...
	if (int(guessedPeaksIndex[0] - 3 * fwhmLeft) < 0 and 
		int(guessedPeaksIndex[-1] + 3 * fwhmRight) > len(xData)): ## case of not enough of points at left and right
	...
	if (int(guessedPeaksIndex[0] - 3 * fwhmLeft) >= 0 and
		int(guessedPeaksIndex[-1] + 3 * fwhmRight) <= len(xData)): ## case of enough of points at left and right
	...
	yCalculatedBackground = np.poly1d(
	backgroundCoefficient)(xData) ## yBackground calcuated with the 1d polynom fitted coefficient
...
def guessParameters(
	...
	#print(peaksGuess)
	if (np.size(peaksGuess) > nbPeaksInBoxes[counterOfBoxes]): ## case if more peaks than expected are detected
		peaksGuess = silx.math.fit.peaks.peak_search(
		yData, 
		fwhmGuess, 
		sensitivity = 1, 
		...
		orderedIndex = np.argsort(
		peaksGuessArray[:,1])[-nbPeaksInBoxes[counterOfBoxes]:]
		peaksGuess = peaksGuessArray[orderedIndex[:], 0] ## peaks indices
	if (np.size(peaksGuess) < nbPeaksInBoxes[counterOfBoxes]): ## case if less peaks than expected are detected
		peaksGuess = silx.math.fit.peaks.peak_search(
		yData, 
		fwhmGuess, 
		sensitivity = 1, 
...
def calibEdd(
	...
	if not 'detectorCalibration' in h5Save.keys():
		...
		'detectorCalibration') ## calibration group
	else:
		calibrationLevel1 = h5Save['detectorCalibration']
	rawDataLevel1_1 = calibrationLevel1.create_group(
	'rawData' + '_' + str(dataset) + '_'
	+ str(scanNumberHorizontalDetector)
	+ '_' + str(scanNumberVerticalDetector)) ## rawData subgroup in calibration group
	fitLevel1_2 = calibrationLevel1.create_group(
	'fit' + '_' + str(dataset) + '_'
	+ str(scanNumberHorizontalDetector)
	+ '_' + str(scanNumberVerticalDetector)) ## fit subgroup in calibration group
	...
	infoGroup = fitLevel1_2.create_group(
	'infos'
	) ## infos group creation
	...
	for index, detector in enumerate(namesDetectors):
		with h5py.File(fileRead, 'r') as h5Read : ## Read the h5 file of raw data
			patternDetector = h5Read[sample + '_' 
			+ str(dataset) + '_' 
			+ str(DetectorScanNumber[index]) + '.1/measurement/' 
			+ detector][()] ## calibration pattern of horizontal detector
	...
	for i in range(numberOfBoxes):
		...
		niterations = 5000, 
		factor = 1,
		anchors = None) ## stripped background of the horizontal detector (obtained by stripping the yData)
		backgroundVerticalDetector = silx.math.fit.strip(
		data = peakVerticalDetector[:,1],
		w = 5,
		niterations = 5000, 
		factor = 1,
		anchors = None) ## stripped background of the vertical detector (obtained by stripping the yData)
		#print(backgroundHorizontalDetector)
		#print(backgroundVerticalDetector)
		fitLevel1_2.create_group(
		f'fitLine_{str(i)}') ## create group for each calibration peak
		...
		dtype = 'float64',
		data = np.transpose((peakHorizontalDetector[:,0],
		...
		dtype = 'float64',
		data = np.transpose((peakVerticalDetector[:,0],
		...
		splitPseudoVoigt(
		peakHorizontalDetector[:,0]
		, optimal_parametersHD) + yCalculatedBackgroundHD))
		...
		splitPseudoVoigt(
		peakVerticalDetector[:,0],
		optimal_parametersVD) + yCalculatedBackgroundVD))
		) ## fitted data of the vertical detector
		fitLevel1_2[f'fitLine_{str(i)}'].create_dataset(
		'errorHorizontalDetector',
		dtype = 'float64',
		data = np.transpose((peakHorizontalDetector[:,0],
		np.absolute(splitPseudoVoigt(
		peakHorizontalDetector[:,0],
		optimal_parametersHD) + yCalculatedBackgroundHD 
		- peakHorizontalDetector[:,1])))
		...
		for n in range(nbPeaksInBoxes[i]):
			fitParamsHD = np.append(fitParamsHD,
			np.append(optimal_parametersHD[5*n:5*n+5],
			100 * np.sum(np.absolute(splitPseudoVoigt(
			peakHorizontalDetector[:,0],
			optimal_parametersHD) + backgroundHorizontalDetector 
			- peakHorizontalDetector[:,1]))
			/ np.sum(peakHorizontalDetector[:,1])), 
			axis = 0) ## 
			fitParamsVD = np.append(fitParamsVD,
			np.append(optimal_parametersVD[5*n:5*n+5],
			100 * np.sum(np.absolute(splitPseudoVoigt(
			peakVerticalDetector[:,0],
			optimal_parametersVD) + backgroundVerticalDetector 
			- peakVerticalDetector[:,1]))
	...
	rawDataLevel1_1.create_dataset('horizontalDetector',
	dtype = 'float64',
	data = patternHorizontalDetector) ## save raw data of the horizontal detector
	rawDataLevel1_1.create_dataset('verticalDetector',
	dtype = 'float64',
	data = patternVerticalDetector) ## save raw data of the vertical detector
	...
	sourceCalibrantFile) ## open source calibration text file
	...
	data = np.transpose((
	curveCalibrationHD[:,0],
	np.abs(np.poly1d(calibCoeffsHD)(curveCalibrationHD[:,0])
	...
	data = np.transpose((
	curveCalibrationVD[:,0],
	np.abs(np.poly1d(calibCoeffsVD)(curveCalibrationVD[:,0])
