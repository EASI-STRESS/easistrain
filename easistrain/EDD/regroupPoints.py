...
#fileRead = ['/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_OR1_TiC.h5','/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_OR2_TiC.h5']
#fileSave = '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_glob_TiC.h5'
#numberOfPeaks = 5
...
def regroupPoints(
	...
	for fileR in fileRead:
		for peakNumber in range(numberOfPeaks):
			if not f'coordInSample_Peak_{str(peakNumber).zfill(4)}' in h5Save.keys():
				...
				pointsInPeakGlobal = h5Save[f'coordInSample_Peak_{str(peakNumber).zfill(4)}']
			if not f'coordInSample_uncertainty_Peak_{str(peakNumber).zfill(4)}' in h5Save.keys():
				...
				upointsInPeakGlobal = h5Save[f'coordInSample_uncertainty_Peak_{str(peakNumber).zfill(4)}']
			with h5py.File(fileR, "r") as h5Read:  ## Read the h5 file of raw data
				shapeScanPoints = h5Read[f'global/inSample_peak_{str(peakNumber).zfill(4)}'].shape[0] ## length
				pointsInPeakGlobal[countFiller:countFiller + shapeScanPoints, :] = h5Read[f'global/inSample_peak_{str(peakNumber).zfill(4)}'][()] ## points of the peak
				upointsInPeakGlobal[countFiller:countFiller + shapeScanPoints, :] = h5Read[f'global/inSample_uncertaintyPeak_{str(peakNumber).zfill(4)}'][()] ## uncertainty of the points of the peak
	...
	for peakNumber in range(numberOfPeaks):
		...
		pointsCounter = 0
		matToFilter = h5Save[f'coordInSample_Peak_{str(peakNumber).zfill(4)}'][()] ## the peak matrix to filter
		umatToFilter = h5Save[f'coordInSample_uncertainty_Peak_{str(peakNumber).zfill(4)}'][()] ## the uncertainty matrix to filter
		matCheck = np.array([[]])
		umatCheck = np.array([[]])
		for i in range(len(matToFilter)):
			matFirstFilter = matToFilter[np.round(matToFilter[:,0], 4) == np.round(matToFilter[i,0], 4),
			:] ## filtering of peak info based on x coordinate of the measurement point
			matSecFilter = matFirstFilter[np.round(matFirstFilter[:,1], 4) == np.round(matToFilter[i,1], 4), 
			:] ## filtering of peak info based on y coordinate of the measurement point
			matThirdFilter = matSecFilter[np.round(matSecFilter[:,2], 4) == np.round(matToFilter[i,2], 4), 
			:] ## filtering of peak info based on z coordinate of the measurement point
			umatFirstFilter = umatToFilter[np.round(umatToFilter[:,0], 4) == np.round(umatToFilter[i,0], 4),
			:] ## filtering of uncertainty based on x coordinate of the measurement point
			umatSecFilter = umatFirstFilter[np.round(umatFirstFilter[:,1], 4) == np.round(umatToFilter[i,1], 4), 
			:] ## filtering of uncertainty based on y coordinate of the measurement point
			umatThirdFilter = umatSecFilter[np.round(umatSecFilter[:,2], 4) == np.round(umatToFilter[i,2], 4), 
