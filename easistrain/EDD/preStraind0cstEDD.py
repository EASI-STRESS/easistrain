...
# fileRead = '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_glob_TiC.h5'
# fileSave = '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_globstrain_TiC.h5'
...
# pathFileAngleCalibration = '/home/esrf/slim/easistrain/easistrain/EDD/Results_ihme10_TiC_calib.h5' 
# scanAngleCalibration = 'fit_0001_3'
# numberOfPeaks = 5
# d0 = [2.491844,2.158000,1.525936,1.301323,1.245922]
...
def uE(dspacing, ttheta, udspacing, uttheta):
	...
	return np.sqrt(((38.4276 * (udspacing**2) * (csctheta**2)) / (dspacing**4))
		+ ((38.4276 * (uttheta**2) * ((1/np.tan(theta))**2) * (csctheta**2)) / (dspacing**2))) ## calculates the uncertainty on the energy coming from d and the fixed diffraction angle


def ud(energy, ttheta, uenergy, uttheta):
	theta = np.radians(0.5 * ttheta)
	csctheta = 1/np.sin(theta)
	return np.sqrt(((38.4276 * (uenergy**2) * (csctheta**2)) / (energy**4))
		+ ((38.4276 * (uttheta**2) * ((1/np.tan(theta))**2) * (csctheta**2)) / (energy**2))) ## calculates the uncertainty on the dspacing coming from the energy and the fixed diffraction angle


def ustrain(energy0, energystrained, uenergy0, uenergystrained):
	return np.sqrt(((uenergy0**2)/(energy0**2))
		+ ((uenergystrained**2)/(energystrained**2))) ## calculates the uncertainty on the strain coming from the measured strain-free energy and the strained energy
...
def diffVector(angles):
    ...
    return q1, q2, q3 ## calcuates the direction of the q vector in the sample reference space using the goniometric angles, diffraction angle and azimuth angle
...
def preStraind0cstEDD(
	...
	strainGroupWithd0 = h5Save.create_group('STRAIN_with_d0') ## creation of the strain group for results with d0
	#strainGroupWithoutd0 = h5Save.create_group('STRAIN_without_d0') ## creation of the strain group for results without d0
	
	with h5py.File(pathFileDetectorCalibration, "r") as h5CalibRead:  ## Read the h5 file of the energy calibration of the detectors
	...
	with h5py.File(pathFileAngleCalibration, "r") as h5AngleRead:  ## Read the h5 file of the angle calibration of the detectors
		AngleHD = h5AngleRead[
		...
		uncertaintyAngleHD = h5AngleRead[
            f"angleCalibration/{scanAngleCalibration}/calibratedAngle/uncertaintyCalibratedAngleHD"
        ][
            ()
        ]  ## import the uncertainty of the calibrated angle of the horizontal detector
		uncertaintyAngleVD = h5AngleRead[
            f"angleCalibration/{scanAngleCalibration}/calibratedAngle/uncertaintyCalibratedAngleVD"
        ][
            ()
        ]  ## import the uncertainty of the calibrated angle of the vertical detector
		
	with h5py.File(fileRead, "r") as h5Read:  ## Read the h5 file of raw data
		for peakNumber in range(numberOfPeaks):
			strainPerPeakWithd0 = strainGroupWithd0.create_group(f'peak_{str(peakNumber).zfill(4)}'
			) ## create group for each peak in the strain group
			for i in range(int(len(list(h5Read[f'pointsPerPeak_{str(peakNumber).zfill(4)}']))/2)):
				allPtsInPeak = h5Read[f'pointsPerPeak_{str(peakNumber).zfill(4)}/point_{str(i).zfill(5)}'][()] ##
				shapeallPtsInPeak = h5Read[f'pointsPerPeak_{str(peakNumber).zfill(4)}/point_{str(i).zfill(5)}'].shape[0] ##
				uallPtsInPeak = h5Read[f'pointsPerPeak_{str(peakNumber).zfill(4)}/uncertaintyPoint_{str(i).zfill(5)}'][()] ##
				...
				for j in range(shapeallPtsInPeak):
					...
					pts[j, 9] = qq1 ## component of the scattering vector in the x direction
					pts[j, 10] = qq2 ## component of the scattering vector in the y direction
					pts[j, 11] = qq3 ## component of the scattering vector in the z direction
					uncertaintyPts[j, 9] = qq1 ## component of the scattering vector in the x direction
					uncertaintyPts[j, 10] = qq2 ## component of the scattering vector in the y direction
					uncertaintyPts[j, 11] = qq3 ## component of the scattering vector in the z direction
