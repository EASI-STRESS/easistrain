...
if (
    len(sys.argv) != 10
):  ### Verification of the number of introduced arguments in the command line
    print("####  The number of arguments is not correct")
    print(
        "#### Arguments to be given: 1)root_data // 2)h5file // 3)scan // 4)detector_name // 5)root_save // 6)save_image_name // 7)extension // 8)int_min // 9)int_max"
    )
    quit()
else:
    ...
    mask(
        ...
        sys.argv[6] + "_mask",
        sys.argv[7],
    )  ### Usage of the mask image to generate a mask for the calibration image
    print(
...
print(
    "#### pyFAI-calib -e (energy in keV)  -p (pixel size in micron) -P 0.95 (Polarization) --npt (nb of points in 1D integrated pattern) --npt-azim (nb of azimuthal sectors) -c (calibrant) -m (mask image) calib_image"
)
